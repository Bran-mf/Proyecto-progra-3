...
public class Mensaje {
    ...
    */
    public int showOptionBotton(String titulo, Window padre, String mensaje){
        int retorno = 0;
        ButtonType bt_con = new ButtonType("OLVIDE MI CONTRASEÑA"),
                   bt_cod = new ButtonType("CORREO DE ACTIVACION"),
        ...
        alert.initOwner(padre);
        alert.setContentText(mensaje);
        alert.getButtonTypes().setAll(bt_cod, bt_con, bt_cancelar);
        Optional<ButtonType> result = alert.showAndWait();
        if(result.get() == bt_cod){
            retorno = 1;
        }else if(result.get() == bt_con){
            retorno = 2;
        }else{
            retorno = 0;
        }
